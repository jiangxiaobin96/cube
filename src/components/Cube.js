...
turn3s(ts);//仅限于3阶魔方,ts是三阶魔方指令的组合。例如 let ts='uu\'bb\'lr\'f';注意字符串中的'要转义
...
function Cube(id, opts) {
    ...
    // console.log("val:"+this.container)
    this.opts = opts || {};//所有信息
...
Cube.prototype.initStyle = function () {
    ...
    for (let i = 0; i < this.boxsDataLength; i++) {
        ...
        //设置没个盒子偏移量
        let x = this.boxsData[i].translateX = (this.boxsData[i].x + 0.5 - (this.order / 2)) * this.boxBorderLength;
        let y = this.boxsData[i].translateY = (this.boxsData[i].y + 0.5 - (this.order / 2)) * this.boxBorderLength;
        let z = this.boxsData[i].translateZ = (this.boxsData[i].z + 0.5 - (this.order / 2)) * this.boxBorderLength;
        let rx = this.boxsData[i].rotateX = 0;
        let ry = this.boxsData[i].rotateY = 0;
        let rz = this.boxsData[i].rotateZ = 0;
...
Cube.prototype.delColor = function () {
    for (let i = 0; i < this.boxsDataLength; i++) {
        let doms = this.boxsData[i].faces;
        for (let j = 0; j < 6; j++) {
...
Cube.prototype.setColor = function (colorarr) {
    ...
    this.colors = colorarr || (function () {
        let arr = [];
        for (let i = 0; i < _this.initColors.length; i++) {
            let subarr = _this.initColors[i].slice(0);
            arr.push(subarr);
        }
        return arr;
    })();
    ...
    for (let i = 0; i < +6; i++) {
        ...
        for (let j = 0; j < Math.pow(this.order, 2); j++) {
            if (!this.colors[i][j]) {
    ...
    let b = this.order - 1;
    let filter = [{y: 0}, {y: b}, {x: 0}, {x: b}, {z: b}, {z: 0}];
    //添加颜色
    for (let i = 0; i < 6; i++) {
        let dom = this.getDomByPos(filter[i]);
...
Cube.prototype.setColorChar = function (color) {
    ...
    let right = color.substr(9, 9);
    let coorright = right[2] + right[5] + right[8] + right[1] + right[4] + right[7] + right[0] + right[3] + right[6];
    let temp = color.substr(0, 9) + coorright + color.substr(18, 36);
    color = temp;

    let bottom = color.substr(27, 9);
    let coorbottom = bottom[6] + bottom[7] + bottom[8] + bottom[3] + bottom[4] + bottom[5] + bottom[0] + bottom[1] + bottom[2];
    temp = color.substr(0, 27) + coorbottom + color.substr(36, 18);
    color = temp;

    let left = color.substr(36, 9);
    let coorleft = left[0] + left[3] + left[6] + left[1] + left[4] + left[7] + left[2] + left[5] + left[8];
    temp = color.substr(0, 36) + coorleft + color.substr(45, 9);
    color = temp;

    let back = color.substr(45, 9);
    let coorback = back[2] + back[1] + back[0] + back[5] + back[4] + back[3] + back[8] + back[7] + back[6];
...
Cube.prototype.getDomByPos = function (filterJson) {
    ...
    let x = /[\d+]/.test(filterJson.x) ? filterJson.x : 'all';
    let y = /[\d+]/.test(filterJson.y) ? filterJson.y : 'all';
    let z = /[\d+]/.test(filterJson.z) ? filterJson.z : 'all';
    for (let i = 0; i < this.boxsDataLength; i++) {
...
Cube.prototype.drag = function (obj, fndown, fnmove, fnup) {
    ...
    let mousedown = function (ev) {
        let oEvent = ev || event;
        let oldX = oEvent.clientX;
        let oldY = oEvent.clientY;
        fndown && fndown();
...
Cube.prototype.trunRightColorChange = function (arr, n) {
    ...
    let cacheArr2 = arr.slice(0);
    for (let i = 0; i < n; i++) {
        cacheArr = cacheArr2.slice(0);
        for (let j = 0; j < length; j++) {
            let dj = (j % order) * order + order - 1 - parseInt(j / order);
...
Cube.prototype.trunTabX = function (arr) {
    if (!arr) return;
    let cacheArr = arr.slice(0);
    let order = this.order;
    let length = arr.length;
    let cacheArr2 = [];
    for (let i = 0; i < length; i++) {
        let dj = (order - 1 - parseInt(i / order)) * order + (i % order);
...
Cube.prototype.trunTabY = function (arr) {
    ...
    let cacheArr2 = [];
    for (let i = 0; i < length; i++) {
        let dj = parseInt(i / order) * order + (order - 1 - i % order);
        cacheArr2[i] = cacheArr[dj];
    }
    return cacheArr2;
};
...
Cube.prototype.reSetColorByTurnEnd = function (coor, num, dir) {
    let nums = [];
    let order = this.order;
    let Morder = order * order;
    let name = ['上', '下', '左', '右', '前', '后'];
    for (let i = 0; i < order; i++) {
        nums.push(i);
    }
    let color0 = this.colors[0].slice(0);
    let color1 = this.colors[1].slice(0);
    let color2 = this.colors[2].slice(0);
    let color3 = this.colors[3].slice(0);
    let color4 = this.colors[4].slice(0);
    let color5 = this.colors[5].slice(0);
//	console.log(color0);
//	console.log(color1);
//console.log(2)
//	console.log(color2);
...
Cube.prototype.turn = function (coor, num, dir, fnComplete) {
    ...
    let filter = {};
    filter[coor] = num;
    let dom = this.seletDoms = this.getDomByPos(filter);
    //添加动作
    let n = 0;//记录转动完成的数
    ...
    for (let i = 0; i < domLength; i++) {
        ...
        //改变ROTATE
        let drx = 0, dry = 0, drz = 0;//默认全部旋转的0度
        let dirNumber = dir ? 1 : -1;
        switch (coor) {
            case 'x':
                drx = 90 * dirNumber;
                break;
...
Cube.prototype.turn3s = function (type, fnComplete) {
    type = type.replace(' ', '');
    let arr = type.match(/\w\'?/g);
    let length = arr.length;
    let now = 0;
    let _this = this;
    let time = this.oneTime;
...
Cube.prototype.random = function (n) {
    ...
    n = n || 30;
    let i = 0;
    let speed = this.oneTime;
    let order = this.order;
    let coors = ['x', 'y', 'z'];
    let linshi = parseInt(1000 / n);
